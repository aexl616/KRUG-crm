...
function saveExpense(data,id='') {
  ...
  saveState();return {ok:true,expense};
}
function deleteExpense(id) {
  if(!canManageExpenses())return false;
  const expense=state.expenses.find(e=>e.id===id);
  if(!expense || !confirm(`Удалить расход «${expense.title}» на ${money(expense.amount)}? Это изменит финансовые итоги.`))return false;
  state.expenses=state.expenses.filter(e=>e.id!==id);saveState();return true;
}
function saveFinancePlan(data) {
  ...
  state.financePlan={monthlyRevenueTarget:KrugFinance.amount(target),monthlyExpenseLimit:KrugFinance.amount(limit)};saveState();return true;
}
function financePeriodControl() {
  return `<label class="f-period">Период<select data-finance-period>${Object.entries(KrugFinance.periods).map(([k,v])=>`<option value="${k}" ${financePeriod===k?'selected':''}>${v}</option>`).join('')}</select></label>`;
}
...
renderFinance=function renderFinanceDashboard() {
  ...
  return `<div class="finance-v2"><header class="f-toolbar"><div><h2>Финансовый центр</h2><p class="muted">${esc(s.period.label)} · факт по ${esc(s.asOf)}</p></div>${financePeriodControl()}<button type="button" class="btn" data-view="expenses">Расходы →</button></header>
  ${financeFeedback?`<p class="f-feedback" role="status">${esc(financeFeedback)}</p>`:''}
  <div class="f-kpis">${financeMetric('Выручка',s.revenue,'Стоимость завершённых записей')}${financeMetric('Поступило',s.cashIn,'Учтённые оплаты за период')}${financeMetric('Расходы',s.expenses,'Оплачено за период')}${financeMetric('Выплачено сотрудникам',s.payoutsPaid,'За период, без запланированных')}${financeMetric('К выплате',s.employeeOutstanding,'Накопленный долг на дату среза')}${financeMetric('Остаток за период',s.netCash,'Поступило − расходы − выплаты')}${financeMetric('Свободно после обязательств',s.freeCashAfterObligations,'Накопленный остаток минус резерв')}${financeMetric('Прогноз месяца',s.forecastFreeCash,'Свободный остаток на конец текущего месяца')}</div>
  <div class="f-grid"><section class="card section f-cash"><h2>Деньги студии</h2><dl><div><dt>Поступило за период</dt><dd>${money(s.cashIn)}</dd></div><div><dt>− Расходы</dt><dd>${money(s.expenses)}</dd></div><div><dt>− Выплаты</dt><dd>${money(s.payoutsPaid)}</dd></div><div class="f-total"><dt>= Остаток за период</dt><dd>${money(s.netCash)}</dd></div><div><dt>Остаток до периода</dt><dd>${money(s.openingCash)}</dd></div><div><dt>Накопленный остаток</dt><dd>${money(s.cashBalanceAsOf)}</dd></div><div><dt>К выплате сотрудникам</dt><dd>${money(s.employeeOutstanding)}</dd></div><div><dt>Резерв обязательств</dt><dd>${money(s.obligationsReserve)}</dd></div><div class="f-total"><dt>Свободно после выплат</dt><dd>${money(s.freeCashAfterObligations)}</dd></div></dl><p class="muted">Резерв по сотруднику — максимум долга и запланированных выплат. План не вычитается второй раз. Остаток рассчитан только по внесённым данным, без банковской сверки.</p></section>
  <section class="card section"><h2>Выручка и оплата записей</h2><dl class="f-ledger"><div><dt>Заработано за период</dt><dd>${money(s.revenue)}</dd></div><div><dt>Получено по этим записям</dt><dd>${money(s.receivedForCompleted)}</dd></div><div><dt>Ещё не получено</dt><dd>${money(s.unpaidRevenue)}</dd></div></dl><p class="muted">Оплаты связаны через ID записи. Предоплата прошлого периода погашает долг; отдельные платежи входят в «Поступило», но не в выручку. Переплата показывается в предупреждениях.</p><h3>Состояние финансов: <span class="f-health" data-health="${s.health}">${s.health}</span></h3><p class="muted">Отрицательный свободный остаток или критические ошибки — риск. Более 80% лимита расходов или другие предупреждения требуют внимания.</p></section>
  <section class="card section"><h2>План / факт текущего месяца</h2><dl class="f-ledger"><div><dt>План выручки</dt><dd>${p.revenueTarget?money(p.revenueTarget):'Не задан'}</dd></div><div><dt>Факт выручки</dt><dd>${money(p.revenueActual)}</dd></div><div><dt>Выполнение</dt><dd>${p.revenuePercent===null?'—':p.revenuePercent+'%'}</dd></div><div><dt>Осталось до плана</dt><dd>${money(p.revenueRemaining)}</dd></div><div><dt>Лимит расходов</dt><dd>${p.expenseLimit?money(p.expenseLimit):'Не задан'}</dd></div><div><dt>Факт расходов</dt><dd>${money(p.expenseActual)}</dd></div><div><dt>Остаток лимита</dt><dd>${p.expenseRemaining===null?'—':money(p.expenseRemaining)}</dd></div></dl><details><summary>Изменить план и лимит</summary><form id="financePlanForm" class="form-grid"><div class="field"><label for="f-target">План выручки, ₽</label><input id="f-target" name="monthlyRevenueTarget" type="number" min="0" step="0.01" required value="${p.revenueTarget}"></div><div class="field"><label for="f-limit">Лимит расходов, ₽</label><input id="f-limit" name="monthlyExpenseLimit" type="number" min="0" step="0.01" required value="${p.expenseLimit}"></div><p class="muted full">0 — план или лимит не задан. Значения используются для текущего месяца; история месячных планов не ведётся.</p><button type="submit" class="btn">Сохранить план</button></form></details></section>
  ...
  <section class="card section"><h2>Обязательства перед сотрудниками</h2><p class="muted">Накопленные суммы по ${esc(s.asOf)}. Текущее правило CRM: начисление равно полной стоимости завершённой записи. Проценты и ставки профиля здесь не применяются.</p><div class="table-wrap" tabindex="0" role="region" aria-label="Обязательства сотрудников, таблица с прокруткой"><table><thead><tr><th>Сотрудник</th><th>Начислено</th><th>Выплачено</th><th>План</th><th>Долг</th><th>Можно запланировать</th><th>Последняя выплата</th></tr></thead><tbody>${s.employees.map(e=>`<tr><td>${esc(e.name)}</td><td>${money(e.earned)}</td><td>${money(e.paid)}</td><td>${money(e.planned)}</td><td>${money(e.outstanding)}</td><td>${money(e.available)}</td><td>${e.lastPayout?esc(String(e.lastPayout.paidAt || e.lastPayout.createdAt).slice(0,10)):'—'}</td></tr>`).join('')}</tbody></table></div><button type="button" class="btn secondary" data-view="payouts">Открыть выплаты команде</button></section>
  <div class="f-grid"><section class="card section"><h2>Расходы по категориям</h2>${financeBars(s.expenseCategories)}<p>Крупнейшая категория: ${esc(s.expenseCategories[0]?.name || '—')}</p><p>Крупнейший расход: ${esc(s.largestExpense?.title || '—')}${s.largestExpense?' · '+money(s.largestExpense.amount):''}</p></section><section class="card section"><h2>Выручка по услугам</h2>${financeBars(s.serviceRevenue)}</section><section class="card section"><h2>Выручка по сотрудникам</h2>${financeBars(s.employeeRevenue)}</section><section class="card section"><h2>Записи и клиенты</h2><p>Завершено записей: ${s.completedBookings}</p><p>Средний чек: ${money(s.averageCheck)}</p><p>Топ-услуга: ${esc(s.topService?.name || '—')}</p><p>Топ-клиент: ${esc(s.topClient?.name || '—')}</p></section></div><section class="card section"><h2>Финансовые предупреждения</h2>${financeWarningsHtml(s.warnings)}</section></div>`;
};
...
function renderExpenses() {
  ...
  <section class="card section"><div class="f-filters">${financePeriodControl()}<label>Категория расхода<select data-expense-category><option value="">Все категории</option>${KrugFinance.categories.map(c=>`<option ${expenseCategoryFilter===c?'selected':''}>${c}</option>`).join('')}</select></label><label>Повторение<select data-expense-recurring><option value="">Все расходы</option><option value="yes" ${expenseRecurringFilter==='yes'?'selected':''}>Ежемесячные</option><option value="no" ${expenseRecurringFilter==='no'?'selected':''}>Разовые</option></select></label><label>Поиск расходов<input data-expense-search value="${esc(expenseSearch)}" placeholder="Название, категория, комментарий"></label></div><div id="financeExpenseList">${expenseListHtml()}</div></section></div>`;
}
...
document.addEventListener('change',event=>{
  ...
  if(el.hasAttribute('data-expense-category')){expenseCategoryFilter=el.value;document.querySelector('#financeExpenseList').innerHTML=expenseListHtml();}
  if(el.hasAttribute('data-expense-recurring')){expenseRecurringFilter=el.value;document.querySelector('#financeExpenseList').innerHTML=expenseListHtml();}
});
document.addEventListener('input',event=>{if(event.target.hasAttribute('data-expense-search')&&canManageExpenses()){expenseSearch=event.target.value;document.querySelector('#financeExpenseList').innerHTML=expenseListHtml();}});
