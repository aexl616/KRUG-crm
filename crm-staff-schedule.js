(() => {
  ...
  document.addEventListener('click', async event => {
    ...
    const cycle=profile.schedule.cycle || {workDays:2,offDays:2,startDate:'',intervals:[{start:'12:00',end:'22:00'}]};
    dialog.innerHTML = `<form class="staff-schedule-form"><div class="staff-schedule-heading"><h2 id="staff-schedule-title">График работы</h2><button type="button" data-close aria-label="Закрыть">×</button></div><p>Московское время. Конец раньше начала — следующий день. Исключение заменяет весь календарный день, включая ночной интервал предыдущего дня.</p>
      ...
      <h3>Регулярный график</h3><label>Тип графика<select name="scheduleMode"><option value="weekly">Недельный</option><option value="cyclic">Циклический</option></select></label><div data-weekly-panel><div class="staff-week">${days.map((_,i)=>dayRow(String(i+1),profile.schedule.weekly[String(i+1)]||[])).join('')}</div></div>
      <div data-cycle-panel hidden><label>Повторение<select data-cycle-preset><option value="2/2">2 рабочих / 2 выходных</option><option value="2/4">2 рабочих / 4 выходных</option><option value="3/3">3 рабочих / 3 выходных</option><option value="4/2">4 рабочих / 2 выходных</option><option value="custom">Свой цикл</option></select></label><div class="staff-exception-add"><label>Рабочих дней<input name="workDays" type="number" min="1" max="366" step="1" required value="${escape(cycle.workDays)}"></label><label>Выходных дней<input name="offDays" type="number" min="1" max="366" step="1" required value="${escape(cycle.offDays)}"></label><label>Первый рабочий день<input name="startDate" type="date" required value="${escape(cycle.startDate)}"></label></div><p>Цикл начинается с этой даты. До неё циклических смен нет. Интервалы повторяются в каждый рабочий день; недельный график сохраняется для переключения обратно.</p><fieldset data-cycle-intervals><legend>Часы рабочего дня</legend><div data-intervals>${intervals(cycle.intervals)}</div><button type="button" data-add-interval>+ Интервал</button></fieldset></div>
    ...
    const mode=dialog.querySelector('[name="scheduleMode"]'),panel=dialog.querySelector('[data-cycle-panel]');
    const preset=dialog.querySelector('[data-cycle-preset]');
    const work=dialog.querySelector('[name="workDays"]'),off=dialog.querySelector('[name="offDays"]');
    const syncPreset=()=>{const key=work.value+'/'+off.value;preset.value=['2/2','2/4','3/3','4/2'].includes(key)?key:'custom';};
    const syncMode=()=>{const cyclic=mode.value==='cyclic';panel.hidden=!cyclic;dialog.querySelector('[data-weekly-panel]').hidden=cyclic;panel.querySelectorAll('input,select,button').forEach(el=>el.disabled=!cyclic);};
    mode.value=profile.schedule.mode || 'weekly';syncMode();syncPreset();
    mode.addEventListener('change',syncMode);
    preset.addEventListener('change',()=>{if(preset.value!=='custom'){[work.value,off.value]=preset.value.split('/');}});
    work.addEventListener('input',syncPreset);off.addEventListener('input',syncPreset);
    ...
    dialog.querySelector('form').addEventListener('submit', async e => {
      ...
      if(mode.value==='cyclic') {
        const slots=[...panel.querySelectorAll('.staff-interval')].map(el=>({start:el.querySelector('[data-start]').value,end:el.querySelector('[data-end]').value}));
        if(!slots.length || slots.some(s=>s.start===s.end)){errorBox.textContent='Добавь рабочий интервал с разным началом и концом.';return;}
        schedule.cycle={workDays:Number(data.get('workDays')),offDays:Number(data.get('offDays')),startDate:data.get('startDate'),intervals:slots};
      } else if(profile.schedule.cycle) schedule.cycle=profile.schedule.cycle;
  ...
  style.textContent='.staff-schedule-dialog{color:#eee;background:#1b1b1b;border:1px solid #444;border-radius:20px;padding:24px;width:min(900px,95vw);max-height:90vh}.staff-schedule-dialog::backdrop{background:#000a}.staff-schedule-heading,.staff-schedule-actions,.staff-interval,.staff-exception-add{display:flex;gap:10px;align-items:center;justify-content:space-between}.staff-schedule-dialog label,.staff-schedule-dialog small{display:block;margin:8px 0}.staff-schedule-dialog input:not([type=checkbox]),.staff-schedule-dialog textarea,.staff-schedule-dialog select{display:block;max-width:100%;background:#282828;color:#eee;border:1px solid #555;border-radius:8px;padding:8px}.staff-schedule-dialog button{cursor:pointer;padding:8px;border-radius:8px}.staff-schedule-dialog fieldset{border:1px solid #444;border-radius:12px;padding:12px;margin:10px 0}.staff-week{display:grid;grid-template-columns:repeat(auto-fit,minmax(270px,1fr));gap:10px}.staff-interval{justify-content:flex-start;margin:8px 0}.staff-schedule-dialog [data-error]{color:#ffa487}.staff-schedule-actions{position:sticky;bottom:-24px;background:#1b1b1b;padding:16px 0}.staff-exception-add{flex-wrap:wrap}';
