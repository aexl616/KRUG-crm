...
test('Mini App bookings cannot be deleted from the CRM mirror', () => {t.run("state.bookings.push({id:'remote',source:'miniapp',miniAppBookingId:'remote-id',employeeId:'u2'});");assert.equal(t.run("canDeleteBooking(state.bookings.find(b=>b.id==='remote'))"),false);});
