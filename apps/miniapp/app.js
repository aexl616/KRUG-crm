(() => {
  ...
  function staffCards() {
    ...
    const any = staffView.mode !== 'required' ? '<button type="button" class="staff-card '+(!bookingDraft.staffId && !bookingDraft.staffInvalid?'selected':'')+'" data-staff="" aria-pressed="'+(!bookingDraft.staffId && !bookingDraft.staffInvalid)+'">Без специалиста<small>Записаться без назначения сотрудника</small></button>' : '';
    return '<section class="staff-selection" aria-label="Выбор специалиста"><h2>Специалист'+(staffView.mode==='optional'?' · по желанию':'')+'</h2><p class="muted">Можно изменить предложенный выбор.</p>'+(staffView.message?'<p role="alert">'+escape(staffView.message)+'</p>':'')+(!staffView.staff.length?'<p role="status">'+(staffView.mode==='optional'?'На это время нет доступных специалистов. Можно записаться без специалиста.':'На это время нет доступных специалистов. Выбери другое время.')+'</p>':'')+any+staffView.staff.map(person=>'<button type="button" class="staff-card '+(bookingDraft.staffId===person.id?'selected':'')+'" data-staff="'+escape(person.id)+'" aria-pressed="'+(bookingDraft.staffId===person.id)+'">'+(/^https:\/\//.test(person.photoUrl||'')?'<img src="'+escape(person.photoUrl)+'" alt="" loading="lazy" referrerpolicy="no-referrer">':'')+'<strong>'+escape(person.name)+'</strong>'+(person.scheduled?'<span class="staff-badge">'+B.staffBadge(bookingDraft.date)+'</span>':'')+(person.experienceSince?'<small>В профессии с '+escape(person.experienceSince)+' года</small>':'')+(person.genres?'<small>'+escape(person.genres)+'</small>':'')+(person.bio?'<small>'+escape(person.bio)+'</small>':'')+'</button>').join('')+'</section>';
  }
