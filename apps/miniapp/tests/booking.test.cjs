...
test('lost create response is recoverable through persisted request ID on reload',async()=>{
 ...
 const y=setup({storage:x.storage});assert.equal((await y.API.getMyBookings())[0].id,remote.id);assert.deepEqual(y.calls[0].body.requestIds,[id]);
});
...
test('cancellation merges partial response with freshly synced server booking',async()=>{
 const x=setup();await x.API.createBooking(draft());x.setReply(async url=>url.endsWith('/sync')?{ok:true,bookings:[{...remote,price:2700}]}:{ok:true,booking:{id:remote.id,requestId:id,status:'cancelled',bonusReserved:0,amountDue:2700}});
...
test('missing Telegram initData blocks booking endpoints',async()=>{
 const x=setup({initData:''});await assert.rejects(x.API.createBooking(draft()),{code:'TELEGRAM_AUTH_REQUIRED'});assert.ok(x.calls.every(c=>c.url.endsWith('/services')));
});
