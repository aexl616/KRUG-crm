-- KRUG 0.10.3 — invalidate public legacy credentials before server auth is deployed.
-- The API validates the existing Mini App admin key first; this RPC is service-role only.

update public.crm_staff_users
set active=false,
    password_hash=extensions.crypt(gen_random_uuid()::text || gen_random_uuid()::text,extensions.gen_salt('bf',11)),
...
create or replace function public.krug_crm_bootstrap_owner(p_login text, p_password text)
returns jsonb
language plpgsql
security definer
...
revoke all on function public.krug_crm_bootstrap_owner(text,text) from public,anon,authenticated;
grant execute on function public.krug_crm_bootstrap_owner(text,text) to service_role;
